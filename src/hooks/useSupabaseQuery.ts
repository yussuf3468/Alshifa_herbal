...
export function useSupabaseQuery<T = any>(
  ...
  return useQuery<T, Error>({
    ...
    staleTime: Infinity, // Stay cached until a mutation invalidates the key (low egress)
    gcTime: 60 * 60 * 1000, // 1 hour - keep cache in memory longer
    refetchOnWindowFocus: false, // Don't refetch on window focus (egress)
    refetchOnMount: true, // Refetch on remount when invalidated → no manual page refresh
    refetchOnReconnect: true, // Refetch after reconnecting
    ...options,
  });
}
...
export function useRecentSales(limit: number = 100) {
  return useSupabaseQueryDirect<any[]>(["recent-sales", limit], () =>
