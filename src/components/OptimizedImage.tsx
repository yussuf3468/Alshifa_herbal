...
interface OptimizedImageProps {
  ...
  alt: string;
  className?: string;
  /** Classes for the outer wrapper div — pass "h-full w-full" when the
   * image should fill a sized container (cards, grids). */
...
const OptimizedImage = memo(
  ({
    ...
    // Fill-style images (h-full) need the wrapper to fill too, otherwise it
    // collapses to auto height and the image can't size against it.
    const fillsContainer = /\bh-full\b/.test(className);

    return (
      <div
        ref={imgRef}
        className={`relative overflow-hidden ${
          fillsContainer ? "h-full w-full" : ""
        } ${containerClassName}`}
        onClick={onClick}
      >
