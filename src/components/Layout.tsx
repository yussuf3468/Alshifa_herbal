...
interface NavItem {
  id: string;
  label: string;
  sublabel?: string; // Optional secondary label shown smaller
  icon: LucideIcon;
  badge?: BadgeKey;
  adminOnly?: boolean;
  staffOnly?: boolean;
...
export default function Layout({
  ...
  // Display name: account full name if set, otherwise the email prefix.
  const staffName = (() => {
  ...
  async function handleLogout() {
    if (confirm("Log out of Al-Shifa Herbal?")) {
      await signOut();
    }
  }
