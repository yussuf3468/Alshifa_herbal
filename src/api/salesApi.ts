...
/**
 * Recent sales only — fetched server-side with a hard LIMIT and a slim column
 * set. Used by the dashboard so it never pulls the entire sales history.
 */
export async function getRecentSales(limit: number = 100) {
  const response = await apiClient
    .from("sales")
    .select(
